    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-purple-50">
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">View and manage washing machines</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {machines.map(machine => {
            ...
            return (
              <GlassCard key={machine.id} className="p-6 cursor-pointer hover:shadow-lg transition-shadow">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3" onClick={() => handleViewMachineDetails(machine)}>
                    <div className={`p-3 rounded-xl ${
                      machine.type === 'washer' 
                        ? 'bg-gradient-to-br from-blue-500 to-blue-600' 
                        : 'bg-gradient-to-br from-purple-500 to-purple-600'
                    }`}>
                      <MachineIcon className="w-6 h-6 text-white" />
                    </div>
                    <div>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg text-sm font-medium ${getStatusColor(machine.status)}`}>
                  ...
                  {machine.currentQR && (
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-900">{machine.currentQR}</p>
                    </div>
                  )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 capitalize">{selectedMachine.type} Machine Details</p>
                  </div>
                </div>
                <button
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg text-sm font-medium ${getStatusColor(selectedMachine.status)}`}>
                      <span className="capitalize">{selectedMachine.status}</span>
                    </div>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-2">Last Updated</p>
                    <p className="text-gray-900">{new Date(selectedMachine.lastUpdated).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-2">Machine Type</p>
                    <p className="text-gray-900 capitalize">{selectedMachine.type}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-500 mb-2">Machine ID</p>
                    <p className="text-gray-900 font-mono text-sm">{selectedMachine.id}</p>
                  </div>
                </div>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div className="space-y-6">
                    ...
                    <div className="bg-blue-50 rounded-lg p-6">
                      ...
                      <div className="grid md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <p className="text-gray-900 font-mono">#{currentAssignment.id.slice(-8).toUpperCase()}</p>
                        </div>
                        <div>
                          <p className="text-sm text-gray-500 mb-2">Current Status</p>
                          <div className={`inline-flex px-3 py-1 rounded-lg text-sm font-medium ${getStatusColor(currentAssignment.status)}`}>
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                          <div className="space-y-3">
                            <div>
                              ...
                              <p className="text-lg font-medium text-gray-900">{studentInfo.name}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="text-gray-900">{studentInfo.phone || 'Not available'}</p>
                            </div>
                          </div>
                          <div className="space-y-3">
                            <div>
                            ...
                            <div>
                              ...
                              <p className="text-gray-900">{studentInfo.rollNumber || 'Not available'}</p>
                            </div>
                            <div>
                              <p className="text-sm text-gray-500">Gender</p>
                              <p className="text-gray-900">{studentInfo.gender || 'Not specified'}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    {/* Laundry Details */}
                    <div className="bg-green-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-gray-900 mb-4">Laundry Bag Details</h3>
                      <div className="grid md:grid-cols-2 gap-6">
                        <div>
                          <p className="text-sm text-gray-500 mb-2">Items ({currentAssignment.items.length})</p>
                          <div className="space-y-2">
                            {currentAssignment.items.map((item, index) => (
                              <div key={index} className="flex items-center gap-2 bg-white rounded-lg p-3 border border-green-200">
                        ...
                        <div className="space-y-4">
                          <div>
                            <p className="text-sm text-gray-500 mb-2">Bag Status</p>
                            <div className="bg-white rounded-lg p-4 border border-green-200">
                              <div className="flex items-center justify-between mb-3">
                                <span className="text-sm font-medium text-gray-900">Current Status</span>
                                <div className={`px-3 py-1 rounded-lg text-xs font-medium ${getStatusColor(currentAssignment.status)}`}>
                                  {currentAssignment.status.replace('_', ' ').toUpperCase()}
                                </div>
                              </div>
                          ...
                          <div>
                            <p className="text-sm text-gray-500 mb-2">QR Codes</p>
                            <div className="space-y-2">
                              {currentAssignment.qrCode && (
                                <div>
                                  <p className="text-xs text-gray-500">Student QR</p>
                                  <p className="text-sm font-mono bg-white px-3 py-2 rounded border">{currentAssignment.qrCode}</p>
                                </div>
                              )}
                              {currentAssignment.bagQRCode && (
                                <div>
                                  <p className="text-xs text-gray-500">Bag QR</p>
                                  <p className="text-sm font-mono bg-white px-3 py-2 rounded border">{currentAssignment.bagQRCode}</p>
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    <div className="bg-blue-50 rounded-lg p-6">
                      <h3 className="text-lg font-semibold text-gray-900 mb-4">Complete Bag Status Timeline</h3>
                      <div className="space-y-3">
                        {[
                          ...
                          return (
                            <div key={step.status} className={`flex items-center gap-4 p-3 rounded-lg ${
                    ...
                    {currentAssignment.specialInstructions && (
                      <div className="bg-yellow-50 rounded-lg p-6">
                        ...
                        <div className="bg-white rounded-lg p-4 border border-yellow-200">
                          <p className="text-gray-900">{currentAssignment.specialInstructions}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {currentAssignment.studentNotes && (
                      <div className="bg-purple-50 rounded-lg p-6">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Student Notes</h3>
                        <div className="bg-white rounded-lg p-4 border border-purple-200">
                          <p className="text-gray-900">{currentAssignment.studentNotes}</p>
                        </div>
                      </div>
                    )}
                    ...
                    {currentAssignment.bagPhoto && (
                      <div className="bg-pink-50 rounded-lg p-6">
                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Bag Photo</h3>
