...
import { User, Mail, Phone, Hash, CheckCircle, Loader2, ArrowLeft, Users } from 'lucide-react';
...
export default function OperatorRegistration() {
    ...
    try {
      ...
      if (result.success) {
        if (result.offline) {
          ...
          showToast('🎉 Welcome to LaundryHub! Your operator account has been created and is pending approval.', 'success');
        }

        setTimeout(() => {
          const dashboardPath = '/operator';
          navigate(dashboardPath);
        }, 1500);
      } else {
  ...
  const getAvailableHostels = () => {
    return ['MH-A', 'MH-B', 'MH-C', 'LH-A', 'LH-B'];
  };
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Operator Registration</h1>
            <p className="text-slate-300">Complete your operator account setup</p>
          </div>
            ...
            <div className="relative p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-slate-300">Operator ID</label>
                ...
                {/* Assigned Hostel */}
                <div className="space-y-2">
                  <label className="block text-sm font-semibold text-slate-300">Assigned Hostel</label>
                  <select
                    id="assignedHostel"
                    name="assignedHostel"
                    value={formData.assignedHostel}
                    onChange={(e) => setFormData({ ...formData, assignedHostel: e.target.value })}
                    required
                ...
                {/* Info Box */}
                <div className="p-4 bg-blue-500/20 border border-blue-400/30 text-blue-300 rounded-2xl backdrop-blur-sm">
                  <div className="flex items-start gap-3">
                    <div className="w-5 h-5 text-blue-400 mt-0.5">ℹ️</div>
                    <div>
                      <h4 className="font-semibold mb-1">Account Status</h4>
                      <p className="text-sm">
                        Your operator account will be set to "pending approval" status.
                        An admin will review and approve your account before you can access operator features.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full px-6 py-4 bg-gradient-to-r from-emerald-600 to-green-700 text-white rounded-xl hover:from-emerald-700 hover:to-green-800 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed font-semibold text-lg"
                >
                  {isLoading ? (
