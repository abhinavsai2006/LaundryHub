      ...
      <div className="container mx-auto px-3 sm:px-4 lg:px-6 py-4 md:py-8">
        <div className="mb-6 md:mb-8">
          <h1 className="text-xl md:text-3xl font-bold text-gray-900 mb-2 flex items-center gap-2">
            ...
            <span className="truncate">Operator Management</span>
          </h1>
          <p className="text-sm md:text-base text-gray-600">Monitor operator activity & workload</p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            ...
            <div className="relative sm:col-span-2 lg:col-span-1">
              ...
              <input
                ...
                className="w-full pl-9 md:pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent text-sm md:text-base min-h-[44px]"
              />
            </div>
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-4 md:p-6">
                      ...
                      <tr key={operator.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-1">
                            {operator.status === 'pending' && (
                              <>
                                <button
                                  ...
                                  className="p-2 text-green-600 hover:text-green-800 hover:bg-green-100 rounded min-h-[36px] min-w-[36px] flex items-center justify-center"
                                ...
                                <button
                                  ...
                                  className="p-2 text-red-600 hover:text-red-800 hover:bg-red-100 rounded min-h-[36px] min-w-[36px] flex items-center justify-center"
                            ...
                            {operator.status === 'active' && (
                              <>
                                <button
                                  ...
                                  className="p-2 text-orange-600 hover:text-orange-800 hover:bg-orange-100 rounded min-h-[36px] min-w-[36px] flex items-center justify-center"
                                ...
                                <button
                                  ...
                                  className="p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-100 rounded min-h-[36px] min-w-[36px] flex items-center justify-center"
                                  title="Assign Hostel"
                                >
                            ...
                            <button
                              onClick={() => setSelectedOperator(operator)}
                              className="p-2 text-purple-600 hover:text-purple-800 hover:bg-purple-100 rounded min-h-[36px] min-w-[36px] flex items-center justify-center"
                              title="View Details"
                            >
          ...
          <div className="md:hidden space-y-3">
            {filteredOperators.map(operator => {
              const stats = getOperatorStats(operator.id);
              return (
                <div key={operator.id} className="border border-gray-200 rounded-lg p-4 bg-white">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-gray-900 text-sm truncate">{operator.name}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3 text-xs">
                    <div>
                      ...
                      <span className="ml-1 font-medium text-gray-900">{operator.hostel || 'Unassigned'}</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Performance:</span>
                      <span className="ml-1 font-medium text-gray-900">{operator.performanceScore}%</span>
                    </div>
                    <div>
                      <span className="text-gray-600">Scans Today:</span>
                      <span className="ml-1 font-medium text-gray-900">{stats.scansToday}</span>
                    </div>
                  </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-3 md:p-4 z-50">
            <div className="bg-white rounded-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-4 md:p-6 border-b border-gray-200 flex items-center justify-between">
                ...
                <button onClick={() => setSelectedOperator(null)} className="p-2 rounded hover:bg-gray-100 min-h-[44px] min-w-[44px] flex items-center justify-center">
                  ×
                </button>
              </div>
              <div className="p-4 md:p-6">
                <div className="mb-4 md:mb-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                    <div className="min-w-0">
                      <h3 className="font-semibold text-gray-900 text-sm md:text-base truncate">{selectedOperator.name}</h3>
                      <p className="text-xs md:text-sm text-gray-600 truncate">{selectedOperator.email}</p>
                    ...
                    {selectedOperator.status === 'active' && (
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <Home className="w-4 h-4 md:w-5 md:h-5 text-blue-600 flex-shrink-0" />
                        <select
                          value={selectedOperator.hostel || ''}
                          onChange={(e) => handleAssignHostel(selectedOperator.id, e.target.value)}
                          className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent font-medium text-sm md:text-base min-h-[44px]"
                        >
                ...
                <div className="mb-4 md:mb-6">
                  <h4 className="font-medium text-gray-900 mb-3 text-sm md:text-base">Recent Scan Logs</h4>
                  <div className="space-y-2 md:space-y-3 max-h-64 md:max-h-96 overflow-y-auto">
                    {laundryItems
                      .filter(item => item.operatorId === selectedOperator.id)
                      .slice(0, 20)
                      .map(item => (
                        <div key={item.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 bg-gray-50 rounded-lg">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-gray-600 truncate">QR: {item.qrCode || 'N/A'}</p>
                            </div>
                          </div>
                    ...
                    {laundryItems.filter(item => item.operatorId === selectedOperator.id).length === 0 && (
                      <p className="text-gray-500 text-center py-4 text-sm md:text-base">No scan logs available</p>
                    )}
                  </div>
                </div>
              </div>
